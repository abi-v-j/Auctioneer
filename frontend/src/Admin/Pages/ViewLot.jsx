...
const ViewLot = () => {
    ...
    },
    {
       ...
       renderCell: (params) => {
         return (
             ...
             <Button variant="outlined" onClick={() => acceptLot(params.row._id)}>Accept</Button>
             <Button variant="outlined" onClick={() => rejectLot(params.row._id)}>Reject</Button>
   ...
   const acceptLot = () => {
    axios.put('http://localhost:5000/Lot').then((response) => {
        console.log(response.data.lot)
        setRows(response.data.lot)
     })
   }
